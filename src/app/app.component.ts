...
export class AppComponent {
  ...
  getTotal(): number {
    ...
    for (let i = 0; i < this.cartitems.length; i++) {
      sum += this.cartitems[i].price.value * this.cartQuantities[this.cartitems[i].ISBN]; 
    }
    return sum;
  }
  ...
  insertBook(e: books) {
    ...
    if (this.cartQuantities[e.ISBN]) {
      this.cartQuantities[e.ISBN]++;
    } else {
      this.cartQuantities[e.ISBN] = 1;
    }
  ...
  removeItem(ISBN: number){
    ...
    this.total -= this.cartitems[itemIndex].price.value * this.cartQuantities[ISBN]; 
    delete this.cartQuantities[ISBN]; 
    this.cartitems.splice(itemIndex,1); 
  }

  increaseQuantity(item: books): void {
    this.cartQuantities[item.ISBN]++; 
    this.total += item.price.value; 
  }
  
  decreaseQuantity(item: books): void {
    if (this.cartQuantities[item.ISBN] > 1) {
      this.cartQuantities[item.ISBN]--; 
      this.total -= item.price.value; 
  }
}
